...
from library import create_gaussian_prior, get_random_X, generate_noise_vector, linear_model, quadratic_model, NN_model, get_theoretical_posterior, get_bin_edges, get_summary_statistics, kl_divergence_gaussians, distance_metric, simulate_output_ABC
...
def get_new_particles(current_weights, current_particles, current_distances, current_summaries_Y, prior_means, prior_covs, X_obs, Y_obs, noise_std, epsilon, M, summary_method_param, JSD_bin_edges, distance_type, model_type, cov_factor=2, epoch=None):
    ...
    noise_matrix, access_indices = create_noise_matrix(N, M, noise_std, GBs=0.1)
    access_indices_position = 0
   
    #Doing the Loop Needed for Step 3 (perturbing + simulating each alive particle until i get MH acceptance)
    for this_index, this_particle in enumerate(current_particles):
        ...
        else:                       #Else constantly try to simulate 
            while(1):
                ...
                new_distances,new_num_accepted, new_summaries_Y = simulate_particle(new_perturbed_particle, X_obs, Y_obs, M, summary_method_param, JSD_bin_edges, distance_type, noise_matrix, access_indices[access_indices_position], model_type, epsilon)
                ...
                if(np.random.rand() <= new_prob_acceptance): 
                    ...
                    break 
    print("simulation time", simulate_time, "pertrub time", perturb_time, "MH_time", MH_time, "times called", simulation_times_called)
    return next_particles, next_distances, next_summaries_Y, simulation_times_called


def simulate_particle(particle, X_obs, Y_obs, M, summary_method_param, JSD_bin_edges, distance_type, noise_matrix, noise_matrix_index, model_type, epsilon=None):
    ...
    if(model_type == "linear"):
        non_noisy_Y = linear_model(X_obs, particle)
    elif(model_type == "quadratic"):
        non_noisy_Y = quadratic_model(X_obs, particle)
    elif(model_type == "NN"):
        non_noisy_Y = NN_model(X_obs, particle)
...
def create_noise_matrix(N, M, noise_std, GBs=0.1):
...
def main():
    ...
    alpha=0.90                                   #Parameter that helps with the threshold adaptivity algorithm
    NT=pop_size/2                                      #Resampling threshold (If ESS<NT resample). 
    epsilon = 1                              #If et < epsilon stop - this is the value he is using too
    ...
    model_type="linear"                                                 #"linear" "quadratic" or "NN"
    if(model_type == "linear"):
        num_params=2
        num_predictors=1
    ...
    print("Running SMC ABC algorithm with pop_size=%d, M=%d, alpha=%.3f, Resampling Threshold=%d, epsilon=%.3f, cov_factor=%.3f, cov_reduction_factor=%.3f noise_std=%.3f, observations_per_dataset=%d, resampling_method=%s, post_process_flag=%d, summary_method_param=%s, SMC_time_limit=%d"
           % (pop_size, M, alpha, NT, epsilon, cov_factor, cov_reduction_factor, noise_std, N, resampling_method, post_process_flag, summary_method_param, smc_time_limit))
    ...
    for i in range(len(Parts0)):
        ...
        current_dists,_,summaries_Y = simulate_particle(Parts0[i], X_obs, Y_obs, M, summary_method_param, JSD_bin_edges, distance_type, noise_matrix, access_indices[access_indices_position], model_type, None)
        access_indices_position +=1 
    ...
    while(1):
        ...
        current_particles, current_dists, current_summaries_Y, current_num_perturbs = get_new_particles(current_weights, current_particles, current_dists, current_summaries_Y,
                                                                                                        ...
                                                                                                        JSD_bin_edges, distance_type, model_type, cov_factor, gen_num) 
    ...
    t_final=time.time()
    print("\nFinished running SMC ABC algorithm with pop_size=%d, M=%d, alpha=%.3f, Resampling Threshold=%d, epsilon=%.3f, cov_factor=%.3f, cov_reduction_factor=%.3f noise_std=%.3f, observations_per_dataset=%d, resampling_method=%s, post_process_flag=%d, summary_method_param=%s, SMC_time_limit=%d"
           % (pop_size, M, alpha, NT, epsilon, cov_factor, cov_reduction_factor, noise_std, N, resampling_method, post_process_flag, summary_method_param, smc_time_limit))
